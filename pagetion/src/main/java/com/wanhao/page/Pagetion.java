 ...
 * 
 */
public class Pagetion extends SimplePage implements Serializable, Pageable {
	private List<?> list;
	...
	public Pagetion(int pageNo, int pageSize, int totalCount, List<?> list) {
		super(pageNo, pageSize, totalCount);
		this.list = list;
	}
	...
	public void pageView(String url, String params) {
		...
		this.pageView
				.add("共<var>"
						+ getTotalPage()
						+ "</var>页 到第<input type='number' id='pageNo' ng-model='pageNo'  ng-change='checkPageNo();' size='3' />页 <input type='button' id='skip' class='hand btn60x20' value='确定' onclick=\"javascript:window.location.href = '"
						+ url + "?" + params
						+ "&pageNo=' + $('#PAGENO').val() \"/>");
	}
}
